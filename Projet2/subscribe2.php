<?php 
    ...
    if ($_SERVER["REQUEST_METHOD"]=="POST"){
        try {
            if(!isset($_POST["ylength"]) || empty($_POST["ylength"])){
        ...
        try {
            if(!isset($users) || empty($users)){
                    throw new Exception("Error: subscribe2.php : user list");
            }
        }  
        catch(Exception $e){
            $_SESSION["msg"]= $e->getMessage();
        ...
        try{
            ...
            foreach($users as $user){
                if($user[0]==$_SESSION["userID"]){ // checks user
                    ...
                    if($user[4]==2){ //if no current subscription
                        ...
                        $interval=new DateInterval('P'.$_POST["ylength"].'Y');
            ...
            if ($try){
                throw new Exception("Error: subscribe2.php : user id");
            } 
